...
struct RenameHostsGroupSheet: View {
    ...
    private func validateName() {
        ...
        if trimmedName != group.name &&
           viewModel.hostsGroups.contains(where: { $0.name == trimmedName }) {
    ...
    private func saveChanges() {
        ...
        viewModel.updateHostsGroup(id: group.id, name: trimmedName, description: trimmedDescription)
        dismiss()
    }
}

#Preview {
    RenameHostsGroupSheet(group: HostGroup(name: "测试分组", description: "测试描述"))
